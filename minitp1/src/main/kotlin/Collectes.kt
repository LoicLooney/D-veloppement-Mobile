...
fun Collecte.resume(): String {
    // A-2 : let : utiliser une valeur si elle n'est pas null
...
fun corrigerPoids(c: Collecte, nouveauPoidsKg: Double): Collecte {
    return (c.copy(poidsKg = nouveauPoidsKg))
...
fun prixEstime(c: Collecte): Double? {
    return (c.produit.prixKg?.times(c.poidsKg) )
...
fun categorieDePoids(c: Collecte): String {
    when{
        c.poidsKg < 10.0 -> return "petite"
        c.poidsKg in 10.1..25.0 -> return "moyenne"
        else -> return "grosse"
    }
...
fun totalParProduit(liste: List<Collecte>): Map<String, Double> {
    return collectes.groupBy { it.produit.toString() }.mapValues { it.value.sumOf { it.poidsKg } }
...
fun collectesValorisables(liste: List<Collecte>): List<Collecte> {
    return collectes.filter { prixEstime(it) != null  }.sortedByDescending { prixEstime(it) }
...
fun producteurLePlusActif(liste: List<Collecte>): Producteur? {
    ...
    // Fonction du Todo
    // return liste.groupBy { it.producteur }.mapValues { it.value.sumOf { it.poidsKg } }.maxByOrNull { it.value }?.key

    // Fonction propose par l'IA
    return liste.groupingBy { it.producteur }
        .fold(0.0) { sum, collecte -> sum + collecte.poidsKg }
